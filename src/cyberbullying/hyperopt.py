"""Optimisation automatique d'hyperparamètres avec Optuna (TPE sampler).

Chaque fonction optimize_* suit le même pattern :
- définir un objective (trial -> score F1 en CV)
- lancer un study Optuna
- retourner best_params + best_score

Optuna est optionnel : si non installé, les fonctions lèvent une ImportError claire.
"""
...
def optimize_logistic_regression(X, y, n_trials: int = 50, cv: int = 5) -> dict[str, Any]:
    ...
    return {"best_params": study.best_params, "best_score": study.best_value, "study": study}
...
def auto_optimize_model(model_name: str, X, y, n_trials: int = 50, cv: int = 5) -> dict[str, Any]:
    """Dispatcher : appelle la bonne fonction d'optimisation selon le nom du modèle."""
    optimizers = {
...
def get_optimization_history(study) -> pd.DataFrame:
    """Historique des trials Optuna sous forme de DataFrame (utile pour plotting)."""
    data = []
