...
Usage:
    ...
    python scripts/run_pipeline.py --stage embed --embedding tfidf
    python scripts/run_pipeline.py --stage download
    python scripts/run_pipeline.py --override /path/to/override.yaml
"""
...
# Override YAML optionnel (chargé depuis --override)
_override_config: dict = {}


def stage_download():
    """Télécharge les datasets de recherche depuis HuggingFace."""
    print("[1/4] download_research")
    import subprocess
    subprocess.run(
        [sys.executable, str(PROJECT_ROOT / "scripts" / "download_research_datasets.py")],
...
def stage_load(n_samples: int | None, data_paths: list[Path] | None = None) -> pd.DataFrame:
    """Charge et fusionne les datasets bruts depuis data/raw/."""
    print("[2/4] load")
    ...
    if not dfs:
        raise RuntimeError("Aucun dataset trouvé. Vérifiez data/raw/ ou sélectionnez des fichiers.")
...
def stage_embed(df: pd.DataFrame, embedding: str, max_features: int = 5000) -> pd.DataFrame:
    """Applique feature engineering + embedding et sauvegarde le CSV résultant."""
    print(f"[3/4] embed ({embedding})")
...
def stage_train(
    ...
    """Entraîne les modèles, logue dans MLflow, et sauvegarde optionnellement un modèle."""
    print("[4/4] train")
    train_cfg = get_train_config()

    # Merge de l'override YAML si fourni
    ...
    # Sauvegarde du vectorizer pour déploiement
    vectorizer = None
    ...
    for key in model_keys:
        ...
        with mlflow.start_run(run_name=f"{key}_{embedding}", log_system_metrics=True):
            ...
            mlflow.log_params({
                "embedding": embedding, "model": key,
                "n_samples": n_samples or len(df_emb),
            ...
            if save_model and key == save_model:
                ...
                print(f"  -> Modèle sauvegardé: {deploy_dir}")
...
def main():
    ...
    parser.add_argument("--save-model", metavar="MODEL")
    ...
    if getattr(args, "data_files", None):
        data_paths = [
            RAW_DATA_DIR / f if not Path(f).is_absolute() else Path(f)
    ...
    if args.stage in ("embed", "all"):
        ...
        if not emb_path.exists():
            print(f"Fichier absent. Lancez d'abord --stage embed --embedding {args.embedding}")
            sys.exit(1)
    ...
    if args.stage in ("train", "all"):
        stage_train(
            ...
            n_samples=args.n_samples, test_size=args.test_size,
            random_state=args.seed, save_model=args.save_model,
