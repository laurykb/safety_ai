...
from cyberbullying.config import EXPERIMENTS_DIR
...
#Etape 2 : Embeddings Word2Vec
from cyberbullying import embed_texts
import numpy as np

print("\n Embedding Word2Vec...")
print("  ⏳ Entraînement du modèle Word2Vec (peut prendre 30-60s)...")
matrix = embed_texts(df_clean['text'].tolist(), 'word2vec', vector_size=300)

# Ajouter les embeddings au DataFrame
embedding_df = pd.DataFrame(
    matrix,
    columns=[f"text_word2vec_{i}" for i in range(matrix.shape[1])]
)
...
output_path = EXPERIMENTS_DIR / "word2vec_test.csv"
...
X = df_embedded[[col for col in df_embedded.columns if col.startswith('text_word2vec')]]
y = df_embedded['type']
...
for model_name, model in models.items():
    ...
    report_filename = results_dir / f"{model_name}_word2vec_report.csv"
