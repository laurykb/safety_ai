...
#Etape 2 : Embeddings Bag of Words
from cyberbullying import embed_texts
import numpy as np

print("\n Embedding Bag of Words...")
matrix = embed_texts(df_clean['text'].tolist(), 'bow', max_features=500)

# Ajouter les embeddings au DataFrame
embedding_df = pd.DataFrame(
    matrix,
    columns=[f"text_bow_{i}" for i in range(matrix.shape[1])]
)
...
output_path = EXPERIMENTS_DIR / "bow_test.csv"
...
X = df_embedded[[col for col in df_embedded.columns if col.startswith('text_bow')]]
y = df_embedded['type']
...
for model_name, model in models.items():
    ...
    report_filename = results_dir / f"{model_name}_bow_report.csv"
