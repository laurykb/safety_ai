...
from cyberbullying.config import EXPERIMENTS_DIR, PRETRAINED_MODELS_DIR
...
#Etape 2 : Embeddings GloVe
from cyberbullying import embed_texts
import numpy as np
from pathlib import Path

print("\n Embedding GloVe...")
print("  ⏳ Chargement du modèle GloVe fine-tuné...")
try:
    # Chemin vers le modèle GloVe fine-tuné
    glove_path = PRETRAINED_MODELS_DIR / "glove_model.pkl"
    
    # GloVe avec le modèle existant
    matrix = embed_texts(
    ...
    embedding_df = pd.DataFrame(
        matrix,
        columns=[f"text_glove_{i}" for i in range(matrix.shape[1])]
    )
    ...
    output_path = EXPERIMENTS_DIR / "glove_test.csv"
    ...
    X = df_embedded[[col for col in df_embedded.columns if col.startswith('text_glove')]]
    y = df_embedded['type']
    ...
    for model_name, model in models.items():
        ...
        report_filename = results_dir / f"{model_name}_glove_report.csv"
    ...
    print(f"  ✗ Erreur GloVe: {e}")
    print("  Note: GloVe nécessite un fichier de modèle pré-entraîné")
