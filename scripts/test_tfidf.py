...
#Etape 2 : Embeddings TF-IDF
from cyberbullying import embed_texts
import numpy as np

print("\n Embedding TF-IDF...")
matrix = embed_texts(df_clean['text'].tolist(), 'tfidf', max_features=500)

# Ajouter les embeddings au DataFrame
embedding_df = pd.DataFrame(
    matrix,
    columns=[f"text_tfidf_{i}" for i in range(matrix.shape[1])]
)
...
output_path = EXPERIMENTS_DIR / "tf_idf_test.csv"
...
X = df_embedded[[col for col in df_embedded.columns if col.startswith('text_tfidf')]]
y = df_embedded['type']
...
for model_name, model in models.items():
    ...
    report_filename = results_dir / f"{model_name}_tfidf_report.csv"
